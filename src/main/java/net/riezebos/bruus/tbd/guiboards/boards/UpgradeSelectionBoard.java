...
import net.riezebos.bruus.tbd.guiboards.boardcreators.UpgradeSelectionBoardCreator;
...
public class UpgradeSelectionBoard extends JPanel implements TimerHolder {
    ...
    private GUITextCollection selectSecondUtility;
    private GUITextCollection selectThirdUtility;
    private GUITextCollection selectFourthUtility;
    private GUITextCollection selectFifthUtility;
    ...
    public UpgradeSelectionBoard() {
        addKeyListener(new UpgradeSelectionBoard.KeyInputReader());
        setFocusable(true);
    ...
    private void initMenuTiles() {
        selectBoonsTitle = UpgradeSelectionBoardCreator.createSelectUpgradesTitleCard();
        firstColumnBackgroundCard = UpgradeSelectionBoardCreator.createFirstColumnBackgroundCard();
        thirdColumnBackgroundCard = UpgradeSelectionBoardCreator.createThirdColumnBackgroundCard();
        fifthColumnBackgroundCard = UpgradeSelectionBoardCreator.createFifthColumnBackgroundCard();
        returnToSelectionBackgroundCard = UpgradeSelectionBoardCreator.createReturnToSelectionBackgroundCard();
        startGameBackgroundCard = UpgradeSelectionBoardCreator.createStartGameBackgroundCard();

        descriptionBackgroundCard = UpgradeSelectionBoardCreator.createDescriptionBackgroundCard();
        descriptionBox = UpgradeSelectionBoardCreator.createDescriptionComponent(descriptionBackgroundCard);

        selectClassButton = UpgradeSelectionBoardCreator.createReturnToClassSelectionButton(returnToSelectionBackgroundCard);
        startGameButton = UpgradeSelectionBoardCreator.createStartGameButton(startGameBackgroundCard);

        selectNepotism = UpgradeSelectionBoardCreator.createNepotismSelectionButton(firstColumnBackgroundCard);
        upgradeNepotism = UpgradeSelectionBoardCreator.createNepotismUpgradeButton(firstColumnBackgroundCard, selectNepotism);

        selectSecondUtility = UpgradeSelectionBoardCreator.createSecondOption(selectNepotism.getComponents().get(0));
        selectThirdUtility = UpgradeSelectionBoardCreator.createThirdOption(selectSecondUtility.getComponents().get(0));
        selectFourthUtility = UpgradeSelectionBoardCreator.createFourthOption(selectThirdUtility.getComponents().get(0));
        selectFifthUtility = UpgradeSelectionBoardCreator.createFifthOption(selectFourthUtility.getComponents().get(0));


        utilityColumnSelectionIndicator = UpgradeSelectionBoardCreator.createUpgradeSelectedCheckmark();
        utilityColumnTitleText = UpgradeSelectionBoardCreator.createFirstColumnTitle(firstColumnBackgroundCard);
        defensiveColumnSelectionIndicator = UpgradeSelectionBoardCreator.createUpgradeSelectedCheckmark();
        defenseColumnTitleText = UpgradeSelectionBoardCreator.createThirdColumnTitle(thirdColumnBackgroundCard);
        offensiveColumnSelectionIndicator = UpgradeSelectionBoardCreator.createUpgradeSelectedCheckmark();
        offenseColumnTitleText = UpgradeSelectionBoardCreator.createFifthColumnTitle(fifthColumnBackgroundCard);

        menuCursor = UpgradeSelectionBoardCreator.createCursor(selectNepotism.getComponents().get(0));
...
//        firstColumn.add(selectNepotism.getComponents().get(0));
        ...
        addToGrid(firstColumn, selectSecondUtility.getComponents().get(0), 0, 1);
        addAllButFirstComponent(selectSecondUtility);

        addToGrid(firstColumn, selectThirdUtility.getComponents().get(0), 0, 2);
        addAllButFirstComponent(selectThirdUtility);

        addToGrid(firstColumn, selectFourthUtility.getComponents().get(0), 0, 3);
        addAllButFirstComponent(selectFourthUtility);

        addToGrid(firstColumn, selectFifthUtility.getComponents().get(0), 0, 4);
        addAllButFirstComponent(selectFifthUtility);
    ...
    private DescriptionInfo prepareDescriptionText(GUIComponent selectedTileHelper) {
        ...
        if (selectedTileHelper.getMenuFunctionality() != null) {
            StartOfGameBoon selectedBoon = UpgradeSelectionBoardCreator.getBoonFromByMenuFunctionality(selectedTileHelper.getMenuFunctionality());
            if (selectedBoon != null) {
                // If the item is a ShopItem, gather its information
                descriptionInfo.title = selectedBoon.getUpgradeName() + " rank: " + selectedBoon.getCurrentLevel();

                if (selectedBoon.isUnlocked()) {
                    descriptionInfo.descriptionText = selectedBoon.getUpgradeDescription();
                } else {
                    descriptionInfo.descriptionText = selectedBoon.getUpgradeUnlockCondition();
                }

                if (selectedBoon.canUpgradeFurther()) {
                    descriptionInfo.cost = "Costs: " + selectedBoon.getUpgradeCost() + " emerald to upgrade. You currently have: " + PlayerProfileManager.getInstance().getLoadedProfile().getEmeralds();
                } else {
...
//        g.setColor(new Color(1.0f, 1.0f, 1.0f, transparency)); // White with transparency
    ...
    private void placeUtilityColumnCheckmark() {
        StartOfGameBoon utilityBoon = StartOfGameBoonsManager.getInstance().getUtilityUpgrade();
        ...
        switch (boonEnums) {
            case NEPOTISM -> {
                checkMarkXCoordinate = selectNepotism.getStartingXCoordinate() + Math.round((selectNepotism.getWidth() / 2));
                checkMarkYCoordinate = selectNepotism.getComponents().get(0).getCenterYCoordinate();
                break;
            }
        }
