...
public class CarrierBossSpawnProtossBeacon implements BossActionable {
    ...
    private Enemy createProtossBeacon(Enemy enemy) {
        ...
        enemyProtossBeacon.getMovementConfiguration().setMovementSpeed(
                enemyProtossBeacon.getMovementConfiguration().getMovementSpeed() * 1 + (Math.min(LevelManager.getInstance().getBossDifficultyLevel() * 0.1f, 0.45f)) //max 45% move speed, cant have it moving too fast
        );
