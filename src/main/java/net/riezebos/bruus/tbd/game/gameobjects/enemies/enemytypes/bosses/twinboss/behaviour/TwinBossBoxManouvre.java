...
public class TwinBossBoxManouvre implements BossActionable {
    ...
    private Point calculateEndDestination() {
    ...
    public boolean activateBehaviour(Enemy enemy1) {
        ...
        if (twinsMoving == TwinBossManager.twinCount && enemy.getMovementConfiguration().getCurrentPath() != null && enemy.getMovementConfiguration().getCurrentPath().getWaypoints().size() <= 1 ) { //or another check wether to see if we reached the destination
            ...
            return false;  //finished but do not return true as it sends a signal to the manager to wipe remaining behaviourx
        }
        return false;
    }
...
//        playSmokeAnimationAtFirstStep(enemy); //Place it at the location the boss teleports back to
        ...
        if(TwinBossManager.twinCount > 4){
            System.out.println("Expected 4 twinbosses but got " + TwinBossManager.twinCount + "");
        }
    ...
    private static void createExplosion(Enemy enemy) {
        ...
        missile.setExplosionSize(1);
        missile.getAnimation().setFrameDelay(1);
        missile.setCreateMissilesOnExplosion(true);
    ...
    private void teleportBossToStartingSpot(TwinBoss boss) {
        if (TwinBossManager.twinCount > 4) {
            OnScreenTextManager.getInstance().addText("TwinBossBoxManouvre expects 4 enemies but got " + TwinBossManager.twinCount + "!");
        }
        if(boss.getPointToTeleportBackTo() == null) {
            ...
            switch (twinsPlacedInPosition) {
                case 0:
                    boss.setCenterCoordinates(Math.round(DataClass.getInstance().getWindowWidth() * 0.1f), Math.round(DataClass.getInstance().getPlayableWindowMaxHeight() * 0.1f));
                ...
                case 1:
                    ...
                    break;
                case 2:
                    boss.setCenterCoordinates(Math.round(DataClass.getInstance().getWindowWidth() * 0.95f), Math.round(DataClass.getInstance().getPlayableWindowMaxHeight() * 0.1f));
                    twinsPlacedInPosition++;
                    break;
                case 3:
            ...
            boss.rotateGameObjectTowards(DataClass.getInstance().getWindowWidth() / 2, DataClass.getInstance().getPlayableWindowMaxHeight() / 2, true);
