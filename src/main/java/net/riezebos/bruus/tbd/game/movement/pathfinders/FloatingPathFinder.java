...
public class FloatingPathFinder implements PathFinder {
    ...
    public Path findPath (GameObject gameObject) {
        ...
        goingUp = !goingUp; //switch between going up/down for next findpath call
        return new Path(pathList, direction);
    }

    private Point calculateNewDestination(GameObject gameObject){
        float yDistanceToFloat = gameObject.getHeight() * 0.1f; //10% of object height
        if(goingUp){
            return new Point(gameObject.getXCoordinate(), gameObject.getYCoordinate() - yDistanceToFloat);
        } else {
            return new Point(gameObject.getXCoordinate(), gameObject.getYCoordinate() + yDistanceToFloat);
        }
