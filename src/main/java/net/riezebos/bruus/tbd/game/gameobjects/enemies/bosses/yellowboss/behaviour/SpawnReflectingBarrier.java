...
public class SpawnReflectingBarrier implements BossActionable {
    ...
    private double intermittenAttackCooldown = 1f;
    ...
    public SpawnReflectingBarrier() {
        ...
        Point upperPoint = new Point(DataClass.getInstance().getWindowWidth() / 2,
                DataClass.getInstance().getWindowHeight() * 0.155);
        ...
        reflectiveMissilesSpawnPoints.add(lowerPoint);
    }
    ...
    public boolean activateBehaviour(Enemy enemy) {
        ...
        if (enemy.isAllowedToFire() && currentTime >= lastAttackTime + attackCooldown && WithinVisualBoundariesCalculator.isWithinBoundaries(enemy)) {
            ...
            if (missileChargingAnimation.getCurrentFrame() >= missileChargingAnimation.getTotalFrames() - 1) {
                if (!allowedToFireBurst) { //Only do it once
                    for (Point spawnLocation : reflectiveMissilesSpawnPoints) {
                        createReflectiveMissiles(enemy, spawnLocation);
                    }
                }
    ...
    private void createReflectiveMissiles(Enemy enemy, Point spawnLocation) {
