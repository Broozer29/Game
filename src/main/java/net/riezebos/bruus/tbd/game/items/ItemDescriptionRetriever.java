...
public class ItemDescriptionRetriever {
    ...
    /*
            ITEM DISTRIBUTION AS OF 01-07-2026 (shared items are included)
                                ...
                                (It's out of date now, a recount is needed)
            CAPTAIN:
                Relics:         7 + 6 + 1   = 14
                Legendaries:    4 + 3 + 1   = 8
                Rares:          4 + 2       = 6
    ...
    public static String getDescriptionOfItem(ItemEnums itemEnums) {
        switch (itemEnums) {
            ...
            case ElectricSupercharger -> {
                return "Your Electro Shred area of effect is improved and deals +" +
            ...
            case ArbiterDamage -> {
                return "Protoss Arbiters no longer heal allies. Protoss Arbiters gain " + Math.round(ArbiterDamage.damageIncreaseMultiplier * 100) + "% increased effectiveness and damage random enemies instead.";
            } //relic
            case InverseRetrieval -> {
                return "Instead of recalling beacons, teleport on top of them. After teleporting release a shockwave dealing " +
                        Math.round(InverseRetrieval.explosionDamageRatio * 100) + "% damage that stuns enemies for " +
            ...
            case EmergencyRepairs -> {
                return "Increases Protoss Ship construction speed by " + Math.round(EmergencyRepairs.constructionSpeedBonusMultiplier * 100) + "% for " + Math.round(EmergencyRepairs.duration) + " seconds after a Protoss Ship dies.";
            } //rare
            ...
            case ArmorPiercingRounds -> {
                return "Increases damage dealt to Mini Bosses by " + Math.round(ArmorPiercingRounds.damageModifier * 100) + "%";
            }
