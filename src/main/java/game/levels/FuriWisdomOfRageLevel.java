...
//		2:13 tot 2:30, langzaam
		...
		EnemySpawnTimer timer = createRandomSpawnTimer(EnemyEnums.Seeker, 1, 138, loopable, Direction.LEFT, scale,
				additionalDelay);
		EnemyFormation formation = formCreator.createFormation(SpawnFormationEnums.Large_smallerthen, 40, 40);
...
//		3:00 tot 3:30, laat ze van alle kanten komen, medium
		addSpawnTimer(createRandomSpawnTimer(EnemyEnums.Alien_Bomb, 50, 190, loopable, Direction.DOWN, scale,
				additionalDelay));
		addSpawnTimer(createRandomSpawnTimer(EnemyEnums.Alien_Bomb, 50, 190, loopable, Direction.UP, scale,
				additionalDelay));
		
		addSpawnTimer(createRandomSpawnTimer(EnemyEnums.Alien_Bomb, 50, 200, loopable, Direction.DOWN, scale,
				additionalDelay));
		addSpawnTimer(createRandomSpawnTimer(EnemyEnums.Alien_Bomb, 50, 200, loopable, Direction.UP, scale,
				additionalDelay));
		...
		for (int i = 184; i < 210; i += 4) {
			addSpawnTimer(
					createRandomSpawnTimer(EnemyEnums.Random, 3, i, loopable, Direction.LEFT_DOWN, scale, additionalDelay));
		}
		
		for (int i = 184; i < 210; i += 4) {
			addSpawnTimer(
					createRandomSpawnTimer(EnemyEnums.Random, 3, i, loopable, Direction.LEFT_UP, scale, additionalDelay));
		}
		...
		for (int i = 184; i < 210; i += 4) {
			addSpawnTimer(
					createRandomSpawnTimer(EnemyEnums.Random, 3, i, loopable, Direction.RIGHT_UP, scale, additionalDelay));
		}
		
		for (int i = 184; i < 210; i += 4) {
			addSpawnTimer(
					createRandomSpawnTimer(EnemyEnums.Random, 3, i, loopable, Direction.RIGHT_DOWN, scale, additionalDelay));
		}
...
//		4:10 tot 4:40 hoog tempo alle kanten
		...
		timer = createRandomSpawnTimer(EnemyEnums.Seeker, 1, 256, loopable, Direction.LEFT_DOWN, scale, additionalDelay);
		formation = formCreator.createFormation(SpawnFormationEnums.Divide, 50, 50);
		timer.setFormation(formation, dataClass.getWindowWidth(), -150);
		addSpawnTimer(timer);
		
		timer = createRandomSpawnTimer(EnemyEnums.Seeker, 1, 262, loopable, Direction.RIGHT_DOWN, scale, additionalDelay);
		...
		addSpawnTimer(timer);
		timer = createRandomSpawnTimer(EnemyEnums.Seeker, 1, 290, loopable, Direction.LEFT_DOWN, scale, additionalDelay);
		formation = formCreator.createFormation(SpawnFormationEnums.Reverse_Divide, 50, 50);
		timer.setFormation(formation, dataClass.getWindowWidth() + 200, -250);
		
		
		addSpawnTimer(timer);
		timer = createRandomSpawnTimer(EnemyEnums.Seeker, 1, 290, loopable, Direction.RIGHT_DOWN, scale, additionalDelay);
		formation = formCreator.createFormation(SpawnFormationEnums.Divide, 50, 50);
